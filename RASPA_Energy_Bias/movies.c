...
void WriteLammpsdata(int i, int cycle)
{
        // here i is the system 
	int j,k,f1,l;
	int atom_A,atom_B, atom_C, atom_D;
	double ref_angle0, ref_angle1, ref_angle2, ref_angle3; // for dihedrals
	int idx;
	char buffer[256];
	FILE *LMPPtr;
	double energy_convert = 1/(4.184*100); // we go with units real in LAMMPS
	VECTOR dr,r,com,com_pdb;
		//for (i = 0; i < NumberOfSystems; i++){
                        sprintf(buffer,"LmpData/System_%d", i);
			mkdir(buffer, S_IRWXU);
			sprintf(buffer,"LmpData/System_%d/cycle_%d.data",i,cycle);
			LMPPtr=fopen(buffer,"w");
			//header of the file 
			fprintf(LMPPtr, "# LAMMPS data file written by WriteLammpsdata function in RASPA(written by Zhao Li)\n");
			int numberofatoms = 0;
			int numberofbonds = 0;
			int numberofbends = 0;
			...
			// no improper right now
			fprintf(LMPPtr, "%i  impropers\n", 0);
			fprintf(LMPPtr, "0.0   %18.12f xlo xhi\n", UnitCellBox[i].ax * NumberOfUnitCells[i].x);
			fprintf(LMPPtr, "0.0   %18.12f ylo yhi\n", UnitCellBox[i].by * NumberOfUnitCells[i].y);
			fprintf(LMPPtr, "0.0   %18.12f zlo zhi\n", UnitCellBox[i].cz * NumberOfUnitCells[i].z);
			fprintf(LMPPtr, "%18.12f %18.12f %18.12f xy xz yz\n", UnitCellBox[i].bx * NumberOfUnitCells[i].y, UnitCellBox[i].cx * NumberOfUnitCells[i].z, UnitCellBox[i].cy * NumberOfUnitCells[i].z);
			int counter = 0;
			int *orders;
			orders=(int*)calloc(NumberOfPseudoAtoms,sizeof(int));
			// count how many types of atoms are there
			...
			fprintf(LMPPtr, "\n");
			fprintf(LMPPtr, "%i atom types\n", counter); 
			// then count how many bond types are there
			counter = 0;
			if (numberofbonds > 0){
			for (j=0;j<NumberOfComponents; j++){
				//fprintf(stderr, "looping over component %i\n", j);
				if (Components[j].NumberOfBonds > 0){  //skip the molecule that has no bonds
					for (k = 0; k < Components[j].NumberOfBonds; k++){
                                                 counter++;
				}
			}
			}
			}			
			fprintf(LMPPtr, "%i bond types\n", counter);
			// then let's count the number of types for angles
			counter = 0;
			if (numberofbends > 0){
			for (j=0;j<NumberOfComponents; j++){
				//fprintf(stderr, "looping over component %i\n", j);
				if (Components[j].NumberOfBends > 0){  //skip the molecule that has no bends (angles)
					for (k = 0; k < Components[j].NumberOfBends; k++){
                                                 counter++;
				}
			}
			}
			}
			fprintf(LMPPtr, "%i angle types\n", counter);
			// finally get the number of types of dihedrals
			counter = 0;
			if (numberoftorsions > 0){
			for (j=0;j<NumberOfComponents; j++){
				//fprintf(stderr, "looping over component %i\n", j);
				if (Components[j].NumberOfTorsions > 0){  //skip the molecule that has no torsions (dihedrals)
					for (k = 0; k < Components[j].NumberOfTorsions; k++){
                                                 counter++;
				}
			}
			}
			}			
			fprintf(LMPPtr, "%i dihedral types\n", counter);
			// no impropers right now!
			fprintf(LMPPtr, "%i improper types\n", 0);
			// first need to count the number of pseudo atoms right
			// need an array to record the order of pseudo atoms
			// get the masses and pair coefficients
			fprintf(LMPPtr, "\nMasses\n");
			fprintf(LMPPtr, "\n");
			counter = 0;
			double mass = 0;
			for (j = 0; j < NumberOfPseudoAtoms; j++){
				...
				if (NumberOfPseudoAtomsType[i][j] > 0){
					counter += 1;
					mass=PseudoAtoms[j].Mass;
					fprintf(LMPPtr, "%i  %18.12f  # %s\n", counter, mass, PseudoAtoms[j].Name);
					orders[j] = counter;
					
				}
			}

			// get the lennard jones parameters for these pseudo atoms
			// needs to convert the energy term to kcal/mol: divide by (4.184*100)
			fprintf(LMPPtr, "\nPair Coeffs\n");
			fprintf(LMPPtr, "\n");
			counter = 0;
			double Param0,Param1;
                        for (j = 0; j < NumberOfPseudoAtoms; j++){
                                if (NumberOfPseudoAtomsType[i][j] > 0){
                                        counter += 1;
                                        Param0=(double)PotentialParms[j][j][0] * energy_convert;
					Param1=(double)PotentialParms[j][j][1];
                                        fprintf(LMPPtr, "%i  %18.12f  %18.12f # %s\n", counter, Param0, Param1, PseudoAtoms[j].Name);

                                }
                        }
			// then get the positions 
			// first loop over all framework atoms
			// all framework atoms belong to group 1, rigid
			int group = 1;
			fprintf(LMPPtr, "\nAtoms\n");
			fprintf(LMPPtr, "\n");
			counter = 0;
			int type = 0; // get the index of that pseudo atom
			double charge = 0;
			double X,Y,Z;
			//char name[256];
			for(f1=0;f1<Framework[i].NumberOfFrameworks;f1++){
				for (j = 0; j < Framework[i].TotalNumberOfAtoms; j++){
					counter++;
					type = Framework[i].Atoms[f1][j].Type;
					charge = Framework[i].Atoms[f1][j].Charge;
					X = Framework[i].Atoms[f1][j].Position.x;
					Y = Framework[i].Atoms[f1][j].Position.y;
					Z = Framework[i].Atoms[f1][j].Position.z;
					//name = PseudoAtoms[type].Name;
					//fprintf(stderr, "name is %s\n", PseudoAtoms[type].Name);
					fprintf(LMPPtr, "%i  %i  %i  %18.5f		%18.5f  %18.5f  %18.5f		# %s\n", counter, group, orders[type], charge, X, Y, Z, PseudoAtoms[type].Name);
				}
			}
			// loop over all components of adsorbates
			// exclude all the fractional molecules and reaction molecules
			// for adsorbates, every molecule is a group
			for(j=0;j<NumberOfAdsorbateMolecules[i];j++){
				if(!(IsFractionalReactionAdsorbateMolecule(j) || IsFractionalAdsorbateMolecule(j))){
					group++;
					...
					for(k=0;k<Adsorbates[i][j].NumberOfAtoms;k++){
						counter++;
						type = Adsorbates[i][j].Atoms[k].Type;
						charge = Adsorbates[i][j].Atoms[k].Charge;
						r.x=MovieScale*(Adsorbates[i][j].Atoms[k].Position.x-dr.x);
						r.y=MovieScale*(Adsorbates[i][j].Atoms[k].Position.y-dr.y);
						r.z=MovieScale*(Adsorbates[i][j].Atoms[k].Position.z-dr.z);
						fprintf(LMPPtr, "%i  %i  %i  %18.5f		%18.5f  %18.5f  %18.5f         # %s %s\n", counter, group, orders[type], charge, r.x, r.y, r.z, Components[Adsorbates[i][j].Type].Name, PseudoAtoms[type].Name);
					}
				}
			}
			// Now let's do bonds
			// print out all the bond coefficients, if there's any bonds
			if (numberofbonds > 0){
			fprintf(LMPPtr, "\nBond Coeffs\n");
			fprintf(LMPPtr, "\n");
			counter = 0;
			for (j=0;j<NumberOfComponents; j++){
				...
				if (Components[j].NumberOfBonds > 0){  //skip the molecule that has no bonds
					for (k = 0; k < Components[j].NumberOfBonds; k++){
                                                 counter++;
                                                 Param0 =  Components[j].BondArguments[k][0] * energy_convert / 2;
                                                 Param1 =  Components[j].BondArguments[k][1];
						 atom_A = Components[j].Type[Components[j].Bonds[k].A]; //get the pseudo atom type of A
						 atom_B = Components[j].Type[Components[j].Bonds[k].B]; //get the pseudo atom type of B
                                                 fprintf(LMPPtr, "%i %18.12f %18.12f #%s %s %s\n", counter, Param0, Param1, Components[j].Name, PseudoAtoms[atom_A].Name, PseudoAtoms[atom_B].Name);
				}
			}
			}
			// then need to print out all the bonds
			fprintf(LMPPtr, "\nBonds\n");
			fprintf(LMPPtr, "\n");
			// now assume framework don't have bonds, count the atoms, should be the same as the positions counter
			counter = Framework[i].TotalNumberOfAtoms + 1;
			int bond_count = 0;
			for(j=0;j<NumberOfAdsorbateMolecules[i];j++){
                                if(!(IsFractionalReactionAdsorbateMolecule(j) || IsFractionalAdsorbateMolecule(j))){
				// get the type of the molecule
				type = Adsorbates[i][j].Type; // going to be 1,2,3,4,5...
				// check if the molecule has a bond
				if (Components[type].NumberOfBonds > 0){
					idx = 1; // bond index
					for (l = type; l > 0; l--){
						//check for molecules before it that has no bonds
						//to get the right index for bond type
						if (Components[l-1].NumberOfBonds > 0) idx += Components[l-1].NumberOfBonds; //get the number of bonds of the molecule before it
					}
					// loop over all bonds in that component
					for (k = 0; k < Components[type].NumberOfBonds; k++){
						bond_count++;
						atom_A =  Components[type].Bonds[k].A + counter;
						atom_B =  Components[type].Bonds[k].B + counter;
						fprintf(LMPPtr, "%i %i %i %i # %s %s %s\n", bond_count, idx + k, atom_A, atom_B, Components[type].Name, PseudoAtoms[Components[type].Type[Components[type].Bonds[k].A]].Name, PseudoAtoms[Components[type].Type[Components[type].Bonds[k].B]].Name);		
					} 
				}
				counter += Components[type].NumberOfAtoms;
				}
			}
			}
			
			// Now let's do angles
			// print out all the angles coefficients, if there's any bends
			// going with harmonic style
			// charmm bends has 4 terms: force constant, reference angle for harmonic part
			// harmonic style only has 2 terms, no ureybradley
			if (numberofbends > 0){
			fprintf(LMPPtr, "\nAngle Coeffs\n");
			fprintf(LMPPtr, "\n");
			counter = 0;
			for (j=0;j<NumberOfComponents; j++){
				...
				if (Components[j].NumberOfBends > 0){  //skip the molecule that has no bends (angles)
					for (k = 0; k < Components[j].NumberOfBends; k++){
                                                 counter++;
                                                 Param0 =  Components[j].BendArguments[k][0] * energy_convert / 2;
                                                 Param1 =  Components[j].BendArguments[k][1] * RAD2DEG;
						 atom_A = Components[j].Type[Components[j].Bends[k].A]; //get the pseudo atom type of A
						 atom_B = Components[j].Type[Components[j].Bends[k].B]; //get the pseudo atom type of B
						 atom_C = Components[j].Type[Components[j].Bends[k].C]; //get the pseudo atom type of C
                                                 fprintf(LMPPtr, "%i %18.12f %18.12f # %s %s %s %s\n", counter, Param0, Param1, Components[j].Name, PseudoAtoms[atom_A].Name, PseudoAtoms[atom_B].Name, PseudoAtoms[atom_C].Name);
				}
			}
			}
			// then need to print out all the bends
			fprintf(LMPPtr, "\nAngles\n");
			fprintf(LMPPtr, "\n");
			// now assume framework don't have bends, count the atoms, should be the same as the positions counter
			counter = Framework[i].TotalNumberOfAtoms + 1;
			int bend_count = 0;
			for(j=0;j<NumberOfAdsorbateMolecules[i];j++){
                                if(!(IsFractionalReactionAdsorbateMolecule(j) || IsFractionalAdsorbateMolecule(j))){
				// get the type of the molecule
				type = Adsorbates[i][j].Type; // going to be 1,2,3,4,5...
				// check if the molecule has a bend
				if (Components[type].NumberOfBends > 0){
					idx = 1; // angle index
					for (l = type; l > 0; l--){
						//check for molecules before it that has no bonds
						//to get the right index for bend type
						if (Components[l-1].NumberOfBends > 0) idx += Components[l-1].NumberOfBends; //get the number of angles of the molecule before it
					}
					// loop over all bonds in that component
					for (k = 0; k < Components[type].NumberOfBends; k++){
						bend_count++;
						atom_A =  Components[type].Bends[k].A + counter;
						atom_B =  Components[type].Bends[k].B + counter;
						atom_C =  Components[type].Bends[k].C + counter;
						fprintf(LMPPtr, "%i %i %i %i %i # %s %s %s %s\n", bend_count, idx + k, atom_A, atom_B, atom_C, Components[type].Name, PseudoAtoms[Components[type].Type[Components[type].Bends[k].A]].Name, PseudoAtoms[Components[type].Type[Components[type].Bends[k].B]].Name, PseudoAtoms[Components[type].Type[Components[type].Bends[k].C]].Name);		
					} 
				}
				counter += Components[type].NumberOfAtoms;
				}
			...
			// now let's count the torsions(dihedrals)
			// Going to follow fourier style of dihedrals, with n = 4
			// for each term, it goes like: K, n, reference degree
			...
			for (j=0;j<NumberOfComponents; j++){
				...
				if (Components[j].NumberOfTorsions > 0){  //skip the molecule that has no torsions (dihedrals)
					for (k = 0; k < Components[j].NumberOfTorsions; k++){
                                                 counter++;
						 // note that you doesn't have to divide by 2 for force constants of dihedrals
                                                 Param0 =  Components[j].TorsionArguments[k][0] * energy_convert;
                                                 Param1 =  Components[j].TorsionArguments[k][1] * energy_convert;
						 Param2 =  Components[j].TorsionArguments[k][2] * energy_convert;
						 Param3 =  Components[j].TorsionArguments[k][3] * energy_convert;
						 atom_A = Components[j].Type[Components[j].Torsions[k].A]; //get the pseudo atom type of A
						 atom_B = Components[j].Type[Components[j].Torsions[k].B]; //get the pseudo atom type of B
						 atom_C = Components[j].Type[Components[j].Torsions[k].C]; //get the pseudo atom type of C
						 atom_D = Components[j].Type[Components[j].Torsions[k].D]; //get the pseudo atom type of D
						 ref_angle0 = 0.0;
						 ref_angle1 = 0.0;
						 ref_angle2 = 180.0;
						 ref_angle3 = 0.0;
						 
                                                 fprintf(LMPPtr, "%i %i %18.12f %i %18.12f %18.12f %i %18.12f  %18.12f %i %18.12f %18.12f %i %18.12f # %s %s %s %s %s\n", counter, 4, Param0, 1, ref_angle0, Param1, 2, ref_angle1, Param2, 3, ref_angle2, Param3, 4, ref_angle3, Components[j].Name, PseudoAtoms[atom_A].Name, PseudoAtoms[atom_B].Name, PseudoAtoms[atom_C].Name, PseudoAtoms[atom_D].Name);
				}
			}
			}
			// then need to print out all the dihedrals
			fprintf(LMPPtr, "\nDihedrals\n");
			fprintf(LMPPtr, "\n");
			// now assume framework don't have dihedrals, count the atoms, should be the same as the positions counter
			counter = Framework[i].TotalNumberOfAtoms + 1;
			int torsion_count = 0;
			for(j=0;j<NumberOfAdsorbateMolecules[i];j++){
                                if(!(IsFractionalReactionAdsorbateMolecule(j) || IsFractionalAdsorbateMolecule(j))){
				// get the type of the molecule
				type = Adsorbates[i][j].Type; // going to be 1,2,3,4,5...
				// check if the molecule has a dihedral
				if (Components[type].NumberOfTorsions > 0){
					idx = 1; // dihedral index
					for (l = type; l > 0; l--){
						//check for molecules before it that has no dihedrals
						//to get the right index for dihedral type
						if (Components[l-1].NumberOfTorsions > 0) idx += Components[l-1].NumberOfTorsions; //get the number of dihedrals of the molecule before it
					}
					// loop over all torsions in that component
					for (k = 0; k < Components[type].NumberOfTorsions; k++){
						torsion_count++;
						atom_A =  Components[type].Torsions[k].A + counter;
						atom_B =  Components[type].Torsions[k].B + counter;
						atom_C =  Components[type].Torsions[k].C + counter;
						atom_D =  Components[type].Torsions[k].D + counter;
						fprintf(LMPPtr, "%i %i %i %i %i %i # %s %s %s %s %s\n", torsion_count, idx + k, atom_A, atom_B, atom_C, atom_D, Components[type].Name, PseudoAtoms[Components[type].Type[Components[type].Torsions[k].A]].Name, PseudoAtoms[Components[type].Type[Components[type].Torsions[k].B]].Name, PseudoAtoms[Components[type].Type[Components[type].Torsions[k].C]].Name, PseudoAtoms[Components[type].Type[Components[type].Torsions[k].D]].Name);		
					} 
				}
				counter += Components[type].NumberOfAtoms;
				}
...
void AllocateMovieMemory(void)
{
  ...
  // also calloc for LMPData variable
  LMPData=(int*)calloc(NumberOfSystems,sizeof(int));
  ...
  //allocate memory for PrintAromaticOrderParameter
  PrintAromaticOrderParameter=(int*)calloc(NumberOfSystems,sizeof(int));
  for(i=0;i<NumberOfSystems;i++)
...
void WriteRestartMovies(FILE *FilePtr)
{
  ...
  fwrite(PrintAromaticOrderParameter,NumberOfSystems,sizeof(int),FilePtr);
  fwrite(LMPData,NumberOfSystems,sizeof(int),FilePtr);
...
void ReadRestartMovies(FILE *FilePtr)
{
  ...
  fread(PrintAromaticOrderParameter,NumberOfSystems,sizeof(int),FilePtr);
  fread(LMPData,NumberOfSystems,sizeof(int),FilePtr);
