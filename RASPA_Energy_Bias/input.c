...
// because I wrote AromaticOrderParameter in molecule_properties.h,
// we need to include it
int *PrintAromaticOrderParameter;
REAL CutOffRingOP;
...
int ReadInput(char *input)
{
  ...
  // add default value for Ring Order Parameters
  CutOffRingOP=9.6; // chose it because of the size of a p-xylene molecule
  ...
  for(i=0;i<NumberOfSystems;i++)
  {
    ...
    LMPData[i]=FALSE;
    PrintAromaticOrderParameter[i]=FALSE;
  ...
  for (line=strtok_r(tmp, "\n", &tokptr); line; line=strtok_r(NULL, "\n", &tokptr))
  {
    ...
    // check if the user want to write LAMMPS data file
    if(strcasecmp("LMPData",keyword)==0)
    {
      if(strcasecmp("yes",firstargument)==0) LMPData[CurrentSystem]=TRUE;
      if(strcasecmp("no",firstargument)==0) LMPData[CurrentSystem]=FALSE;
    }
    // check if the user want to print out the order parameters for Aromatics
    if(strcasecmp("PrintAromaticOrderParameter",keyword)==0)
	{
	    if(strcasecmp("yes",firstargument)==0) PrintAromaticOrderParameter[CurrentSystem]=TRUE;
        if(strcasecmp("no",firstargument)==0) PrintAromaticOrderParameter[CurrentSystem]=FALSE;
    }
	
    // add cutoff for considering AromaticOrderParameter
    if(strcasecmp("CutOffRingOP",keyword)==0) sscanf(arguments,"%lf",&CutOffRingOP);
  ...
  if(Framework[0].FrameworkModel==GRID)
  {
    ...
    // Added by Zhao: Since sometimes grids are not accurate, throw away when it is not accurate
    // Initialize with TRUE, then when testing, turn inaccurate ones to FALSE
    for(i=0;i<NumberOfPseudoAtoms;i++)
    {
      PseudoAtoms[i].GridAccurate=TRUE;
    }
