...
#include <dirent.h>   // also for making new dirs
#include <sys/stat.h> // for making directories
#include <errno.h>    // writing error note
...
void PrintIntervalStatusInit(long long CurrentCycle,long long NumberOfCycles,FILE *FilePtr)
{
  ...
  // add line for RingOP
  if(PrintAromaticOrderParameter[CurrentSystem])
  {
   int Cycle_type;
   Cycle_type=0;
   DIR* dir = opendir("RingOrderParameter");
   if(dir)
   {
     closedir(dir);
   }
   else if(ENOENT == errno){
   // check if there's no such directory
     mkdir("RingOrderParameter",S_IRWXU);
   }

   ComputeAromaticOrderParameter(CurrentSystem, CurrentCycle, Cycle_type);
   if(RingOrderPairs < 5)
   {
     fprintf(FilePtr,"\n\nThere are not enough pairs for Ring Order Parameter.\n");
   }
   else
   {
     fprintf(FilePtr,"\n\nThere are %i pairs, RingOP: % 22.10lf\n", RingOrderPairs, (double)RingOrderParam);
   }
  }
...
void PrintIntervalStatusEquilibration(long long CurrentCycle,long long NumberOfCycles,FILE *FilePtr)
{
  ...
  {
   int Cycle_type;
   Cycle_type=1; // Equilibration cycle
   DIR* dir = opendir("RingOrderParameter");
   if(dir)
   {
     closedir(dir);
...
*/
  ...
  for(j=0;j<NumberOfComponents;j++)
  {
    ...
    fprintf(FilePtr,"\tAverage loading absolute [gram/Liter framework]    %18.10lf +/- %18.10lf [-]\n",
      (double)(1/22.4*Components[j].Mass*sum*Components[j].MOLEC_PER_UC_TO_CC_STP_CC[CurrentSystem]/((REAL)nr*(REAL)NR_BLOCKS)),
      (double)(1/22.4*Components[j].Mass*2.0*Components[j].MOLEC_PER_UC_TO_CC_STP_CC[CurrentSystem]*sqrt(fabs((sum2/(REAL)NR_BLOCKS)-SQR(sum)/(REAL)SQR(NR_BLOCKS)))/(REAL)nr));
    fprintf(FilePtr,"\n");
