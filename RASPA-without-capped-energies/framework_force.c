...
void CalculateFrameworkAdsorbateVDWForce(void)
{
  ...
  for(i=0;i<NumberOfAdsorbateMolecules[CurrentSystem];i++)
  {
    ...
    for(j=0;j<Adsorbates[CurrentSystem][i].NumberOfAtoms;j++)
    {
      ...
      // modified by Zhao: For EB, for example in benzene, we used effective first bead (fictional),
      // we must turn grids OFF when computing energies for the effective first bead, because grids are used just
      // for getting weights of grid points.
      // first get the first bead pseudo atom of molecule i
      FirstBeadType=0;
      if(!UseEBCBMCEnergyFirstBead) // if not use 1st bead energy, then 1st bead energy-grid is just for computing weight of cubelets
      {
        FirstBeadType=Components[Adsorbates[CurrentSystem][i].Type].Type[Components[Adsorbates[CurrentSystem][i].Type].StartingBead];
      }

      if((Framework[CurrentSystem].FrameworkModel==GRID)&&(VDWGrid[typeA])&&(!IsFractionalAdsorbateMolecule(i))&&(!(typeA==FirstBeadType)))
      {
