...
int HandleFirstBead(int Switch)
{
  ...
  if(UseTabularGrid && EBCBMC && Switch==CBMC_INSERTION && BiasingMethod==LJ_BIASING) 
  {
    //if (CheckingEBCB == 0){
     // CheckingEBCB++;
      //fprintf(stderr, "Insertion: This simulation is Energy Biased\n");
   //}
    ...
    if (UseEBCBMCEnergyFirstBead) 
    {
      ...
      StoredR=SumVDWGridRosenbluth[type]-exp(-Beta[CurrentSystem]*VDWGridEnergy);

      //fprintf(stderr, "Grid Index: %i %i %i\n", EBCBMCGridLocation[type][i][j].x, EBCBMCGridLocation[type][i][j].y, EBCBMCGridLocation[type][i][j].z);
      //fprintf(stderr, "VDW Grid Energy is: %lf\n", VDWGrid[type][EBCBMCGridLocation[type][i][j].x][EBCBMCGridLocation[type][i][j].y][EBCBMCGridLocation[type][i][j].z][0]);
      //fprintf(stderr, "VDW Grid Energy is: %lf\n", VDWGridEnergy);

      //fprintf(stderr, "VDWs are: %lf %lf %lf\n", EnergyHostVDW, EnergyCationVDW, EnergyAdsorbateVDW);
      //fprintf(stderr, "Charge Energies are: %lf %lf %lf\n", EnergyHostChargeCharge, EnergyAdsorbateChargeCharge, EnergyCationChargeCharge);
      //fprintf(stderr, "Dipoles are: %lf %lf %lf\n", EnergyHostChargeBondDipole, EnergyAdsorbateChargeBondDipole, EnergyCationChargeBondDipole);
      ...
      //fprintf(stderr, "Rosenbluth of 1st bead is: %lf\n", RosenBluthFactorFirstBead);
    }
    else 
    {
      FirstBeadPosition=posA;
      ...
      // calculate energies
      // since we are not using energy of the first bead, we change to the other function
	  // for CFCMC, we need to change lambda to 1.
      //EnergyHostVDW=CalculateFrameworkVDWEnergyAtPositionEBFakeFirstBead(posA,type,(REAL)(1.0));
      //fprintf(stderr, "CFVDWScaling is %lf\n", CFVDWScaling[start]);
      //fprintf(stderr, "EHVDW is %lf\n", EnergyHostVDW);
      //fprintf(stderr, "SumVDWGridRosenbluth is %lf\n", SumVDWGridRosenbluth[type]);
      //fprintf(stderr, "EBCBMCVolume is %d\n", EBCBMCVolume);
      //Caculate Rosenbluth factor for the first bead
      //fprintf(stderr, "CBMC_INSERTION\n");
      //fprintf(stderr, "VDWGridEnergy is %lf\n", VDWGridEnergy);
      RosenBluthFactorFirstBead=SumVDWGridRosenbluth[type]*(1.0/(REAL)(EBCBMCVolume))*exp(-Beta[CurrentSystem]*(-VDWGridEnergy));
      //fprintf(stderr, "Rosenbluth of 1st bead is: %lf\n", RosenBluthFactorFirstBead);
    }
  }
  else if (UseTabularGrid && EBCBMC && (Switch==CBMC_DELETION || Switch==CBMC_RETRACE_REINSERTION || Switch==CBMC_PARTIAL_INSERTION) && BiasingMethod==LJ_BIASING)
  {
    //if (CheckingEBCB == 0){
      //CheckingEBCB++;
      //fprintf(stderr, "Deletion: This simulation is Energy Biased\n");
    //}    
    ...
    if (UseEBCBMCEnergyFirstBead) 
    {
      ...
      //fprintf(stderr, "VDW Grid Energy is: %lf\n", VDWGrid[type][GridIndex.x][GridIndex.y][GridIndex.z][0]);
    ...
    else 
    {
      ...
      //fprintf(stderr, "EBCBMCVolume is %d\n", EBCBMCVolume);
      //
      //fprintf(stderr, "CBMC_DELETION or Reinsertion\n");
      GridIndex=ConvertXYZPositionToGridIndex(posA);     
      //fprintf(stderr, "GridENERGY is %lf\n", VDWGrid[type][GridIndex.x][GridIndex.y][GridIndex.z][0]);
     //Caculate Rosenbluth factor for the first bead
      RosenBluthFactorFirstBead=(SumVDWGridRosenbluth[type]/(REAL)(EBCBMCVolume))*exp(-Beta[CurrentSystem]*(-VDWGrid[type][GridIndex.x][GridIndex.y][GridIndex.z][0]));
      //fprintf(stderr, "Rosenbluth of 1st bead is: %lf\n", RosenBluthFactorFirstBead);
    }
  }
  else 
  {
    ...
    else if(Switch==CBMC_RETRACE_REINSERTION)
    {
      ...
      i=0;
      //fprintf(stderr, "CBMC_RETRACE_REINSERTION, Rosen_before is %lf\n", RosenBluthFactorFirstBead);
      // w_1(o)=exp(-beta u_1(0)+r  Eq. 18 from Esselink et al.
      RosenBluthFactorFirstBead+=StoredR;
      //fprintf(stderr, "StoredR is %lf\n", StoredR);
      //fprintf(stderr, "RosenBluthFactorFirstBead is %lf\n", RosenBluthFactorFirstBead);
    }
    else
      i=0;
