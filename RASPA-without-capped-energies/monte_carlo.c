...
void MonteCarloSimulation(void)
{
  ...
  for(CurrentIsothermPressure=0;CurrentIsothermPressure<NumberOfIsothermPressures;CurrentIsothermPressure++)
  {
	...
	// LAMMPS data and Ring Parameter stuff
        DIR* dir = opendir("LmpData");
        if(dir){
          closedir(dir);
        } else if(LMPData[CurrentSystem]&&(ENOENT == errno)){
          // check if there's no such directory
          mkdir("LmpData",S_IRWXU);
        }
        // check if the user wants to print LAMMPS data
        if(LMPData[CurrentSystem]&&(CurrentCycle%WriteMoviesEvery[CurrentSystem]==0)){
          WriteLammpsdata(CurrentSystem, CurrentCycle);
        }
        // check if AromaticParameter folder exist
        /*dir = opendir("RingOrderParameter");
        if(dir){
          closedir(dir);
        } else if(PrintAromaticOrderParameter[CurrentSystem]&&(ENOENT == errno)){
          // check if there's no such directory
          mkdir("RingOrderParameter",S_IRWXU);
        }
        // check if the user wants to calculate aromatic order parameters
        if(PrintAromaticOrderParameter[CurrentSystem]&&(CurrentCycle%WriteMoviesEvery[CurrentSystem]==0)){
          ComputeAromaticOrderParameter(CurrentSystem, CurrentCycle);
        }
        */
      }
